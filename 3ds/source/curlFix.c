...
// Touch mode settings
#define TOUCH_MODE_THRESHOLD 1       // Minimum pixel change for touch mode
#define TOUCH_MODE_THROTTLE_TIME 2083334  // ~120fps in system ticks
...
#define SUBPIXEL_PRECISION 4  // 4 bits of subpixel precision (1/16 pixel)
static int touchPrecisionX = 0;  // Fixed point X (actual value × 16)
static int touchPrecisionY = 0;  // Fixed point Y (actual value × 16)
...
// Touch reliability settings
#define TOUCH_START_RELIABILITY 1  // Only use 2 retries for initial touch
#define TOUCH_START_RETRY_DELAY 0  // 2ms between retries for touch start
...
void sendModeSyncPacket(u64 currentTime) {
    ...
    if (currentTime - lastModeSyncTime > MODE_SYNC_INTERVAL) {
        ...
        // Send mode sync packet directly
        sendto(sock, syncPacket, strlen(syncPacket), 0, 
              (struct sockaddr*)&serverAddr, sizeof(serverAddr));
    ...
    while (aptMainLoop()) {
        ...
        if ((keys & TOUCH_MODE_BUTTONS) == TOUCH_MODE_BUTTONS) {
            ...
            if (currentTime - touch_mode_last_toggle > TOUCH_MODE_DEBOUNCE_TIME) {
                ...
                // Send mode update packet
                char modeChangePacket[64];
                buttonsToString(keys, pos, touch, modeChangePacket);
                
                // Use direct send
                sendto(sock, modeChangePacket, strlen(modeChangePacket), 0,
                      (struct sockaddr*)&serverAddr, sizeof(serverAddr));
            }
        }
        ...
        // MODIFIED TOUCH MODE SECTION
        // Simplified touch handling for touch mode
        if (touch_mode_active && touchActive) {
            // In touch mode, always send updates for active touches
            // No throttling or threshold checks - pure responsiveness
            shouldSendUpdate = true;
            
            // We still need to update precision tracking for other parts of the code
            touchPrecisionX = touch.px << SUBPIXEL_PRECISION;
            touchPrecisionY = touch.py << SUBPIXEL_PRECISION;
        } else {
            // Normal mode logic remains unchanged
        ...
        // MODIFIED SENDING SECTION
        // Replace the conditional packet sending logic with simpler direct sending
        if (shouldSendUpdate) {           
            // Update last states
            lastKeys = keys;
            lastPos = pos;
            ...
            // Always use simple send - no reliability mechanism or bursts
        ...
        // MODIFIED SLEEP TIMING
        // In touch mode, use consistently short sleep time
        if (touch_mode_active) {
            // Consistently fast timing - no differentiation between first touch and continued
            svcSleepThread(250000); // 0.25ms - Ultra responsive (~4000fps theoretical)
        } else {
            svcSleepThread(8000000); // 8ms - More responsive normal mode (~125fps)
        }
    }
