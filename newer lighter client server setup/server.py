...
print("- Circle pad mapped to X/Y axes")
...
while True:
    try:
        ...
        if match:
            ...
            # Then handle circle pad - note how we're forcing it to center (16384) if it's close
            # This helps prevent "sticky" inputs where the circle pad stays slightly off-center
            center_threshold = 1  # Adjust as needed
            if abs(cpad_x - 16384) < center_threshold:
                cpad_x = 16384  # Force to exact center
            if abs(cpad_y - 16384) < center_threshold:
                cpad_y = 16384  # Force to exact center

            # Now set the axis values
            setAxis(1 + offsetA, cpad_x)
            setAxis(2 + offsetA, 32768 - cpad_y)  # Invert Y-axis
