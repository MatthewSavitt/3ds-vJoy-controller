...
# Screenshot handling
screenshot_prefix = "S:"  # Must match the prefix in the 3DS code
...
info_text.insert(tk.END, "- L+R+Select+Start = Take screenshot\n")
...
def send_screenshot_to_3ds(sock, addr, screenshot):
    """Send screenshot with enhanced reliability for 3DS"""
    try:
        # Pre-scale to 3DS resolution
        width, height = 320, 240
        screenshot_resized = screenshot.resize((width, height), Image.Resampling.LANCZOS)
        
        # Convert directly to RGB565 (2 bytes per pixel)
        pixels = []
        for y in range(height):
            for x in range(width):
                r, g, b = screenshot_resized.getpixel((x, y))[:3]
                # RGB565: RRRRRGGGGGGBBBBB
                rgb565 = ((r >> 3) << 11) | ((g >> 2) << 5) | (b >> 3)
                # Store in little-endian (low byte first)
        ...
        chunk_size = 384  # Match the client's chunk size
        total_chunks = (total_size + chunk_size - 1) // chunk_size
        
        log_message(f"Sending screenshot: {width}x{height}, {total_size} bytes, {total_chunks} chunks")
        
        # Send header packet multiple times for reliability
        header = struct.pack('<IIHH', total_size, total_chunks, width, height)
        header_packet = b"IMGSTART:" + header
        
        # Send header 3 times
        for i in range(3):
            sock.sendto(header_packet, addr)
            time.sleep(0.02)
        
        # Wait to ensure header is processed
        time.sleep(0.1)
        ...
        for i in range(total_chunks):
            ...
            chunk_data = img_data[offset:end_offset]
            
            # Packet format: sequence_number + data
            packet = struct.pack('<I', i) + chunk_data
            full_packet = b"IMGCHUNK:" + packet
            
            # Send critical chunks (first few and last few) twice
            if i < 5 or i >= total_chunks - 5:
                for _ in range(2):
            ...
            # Progress reporting
            if chunks_sent % 20 == 0 or chunks_sent == total_chunks:
                progress = (chunks_sent / total_chunks) * 100
                log_message(f"Progress: {chunks_sent}/{total_chunks} chunks sent ({progress:.1f}%)")
            
            # Slower pacing for 3DS
            time.sleep(0.01)
        
        # Send end packet multiple times
        end_packet = b"IMGEND:"
        for i in range(3):
            sock.sendto(end_packet, addr)
            time.sleep(0.02)
        
        log_message("Screenshot transmission complete")
        
    except Exception as e:
        log_message(f"Error sending screenshot: {e}")
        import traceback
...
def handleScreenshot(sock, addr):
    """Handle screenshot request from 3DS - IMPROVED VERSION"""
    try:
        log_message(f"Screenshot requested from {addr[0]}:{addr[1]}")
        
        # Capture screenshot
        screenshot = pyautogui.screenshot()
        
        # Send the screenshot to the 3DS
        send_screenshot_to_3ds(sock, addr, screenshot)
        
        # Don't save locally anymore
        
    except Exception as e:
        log_message(f"Error handling screenshot: {e}")
        import traceback
        log_message(traceback.format_exc())
...
def server_loop():
    ...
    while True:
        try:
            ...
            # Check if this is a screenshot request packet
            if data_str.startswith(screenshot_prefix):
                handleScreenshot(sock, addr)
                continue
