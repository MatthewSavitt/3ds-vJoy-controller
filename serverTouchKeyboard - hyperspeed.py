...
touch_mode_smoothing_initial = 0.1  # Even less smoothing for first stroke (NEW LINE)
touch_mode_initial_strokes = 3  # Number of updates with reduced smoothing (NEW LINE)
...
def handleMouse(touch_active, touch_x, touch_y, buttons, client_mode=None):
    ...
    global stroke_counter  # NEW LINE
    
    current_time = time.time()
    
    # Touch mode toggle handling (no changes)
    BTN_L = 1 << 8
    BTN_R = 1 << 9
    BTN_SELECT = 1 << 10
    
    # Client mode sync handling (no changes)
    ...
    if touch_mode_active:
        ...
        if touch_active:
            # If first touch or position needs init - MODIFIED SECTION
            if last_mouse_x is None:
                # Go directly to target position without any smoothing for first touch
                pyautogui.moveTo(target_x, target_y, _pause=False)
                last_mouse_x = target_x
                last_mouse_y = target_y
                stroke_counter = 1  # Start counting initial strokes
                if debug_mode:
                    log_message("First touch - direct positioning")
            else:
                # Apply smoothing for continued movement - MODIFIED SECTION
                # Use less smoothing for first few updates after pen down
                if stroke_counter > 0 and stroke_counter < touch_mode_initial_strokes:
                    # Use reduced smoothing for initial strokes
                    smooth_factor = touch_mode_smoothing_initial
                    stroke_counter += 1
                    if debug_mode:
                        log_message(f"Initial stroke {stroke_counter} - reduced smoothing")
                else:
                    # Use normal smoothing for continuous movement
                    smooth_factor = touch_mode_smoothing
                
                # Apply appropriate smoothing factor
        ...
        # Handle left click based ONLY on touch_active (unchanged)
        # NEW FEATURE: If L is held in touch mode, only move cursor (disable clicking)
        if touch_active and l_button_pressed:
            ...
            if mouse_l_pressed:
                ...
                mouse_l_pressed = False
                if debug_mode:
                    log_message("L button held - CURSOR TRACKING MODE")
            if mouse_r_pressed:
                pyautogui.mouseUp(button='right')
                mouse_r_pressed = False
        ...
        if not touch_active:
            last_mouse_x = None
            last_mouse_y = None
            stroke_counter = 0  # Reset stroke counter when pen is lifted
